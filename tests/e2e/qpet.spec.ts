...
test('normalizes provider events and prioritizes the floating pet activity tray', async () => {
  ...
  try {
    ...
    const petWindowBehavior = await electronApp.evaluate(({ BrowserWindow }) => {
    ...
    expect(petWindowBehavior).toEqual({ alwaysOnTop: true, visibleOnAllWorkspaces: true })
    ...
    const trayTypography = await tray.evaluate(() => {
      ...
      return {
        brand: readFontSize('.brand-lockup strong'),
        status: readFontSize('.status-banner strong'),
        activity: readFontSize('.activity-title strong')
      }
    })
    expect(trayTypography).toEqual({ brand: 18, status: 16, activity: 14 })
